...
int menu(int& refmoney, int& refeda, int& refson, int& refmood, int& refstudy) {
    ...
    if (studyerror == 1) {
        cout << "У ТЕБЯ СЛИШКОМ МАЛЕНЬКИЙ УРОВЕНЬ УЧЕБЫ ДЛЯ ТАКОЙ РАБОТЫ" << endl;
    }
    if (vtoroedihanie == 2) {
    ...
    if (refeda <= 0) {
        death(refmoney, refeda, refson, refmood, refstudy);
    }
    if (refson <= 0) {
        death(refmoney, refeda, refson, refmood, refstudy);
    }
    ...
    //если параметры превысят 100процентов
    if (refeda > 100) {
        refeda = 100;
    }
    if (refson > 100) {
...
int main() {
    setlocale(LC_ALL, "ru");
    //menu(refmoney, refeda, refson, refmood);
    ...
    // УВЕЛИЧИВАЕМ РАЗМЕР (масштабируем):
    font.dwFontSize.Y = 24;  // Было обычно 16, стало 24 (увеличили)
    // или вот так в 1.5 раза:
    // font.dwFontSize.Y = font.dwFontSize.Y * 3 / 2;
